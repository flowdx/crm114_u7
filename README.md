...
Der Rspamd-Score, den die Ubernauten für jede E-Mail automatisch bereitstellen, wird im Rahmen dieses Tutorials nicht verwendet. CRM114 verrichtet seine Arbeit so gut, dass dies nicht notwendig erscheint. Eine Kombination aus CRM114 und Rspamd wäre natürlich auch möglich, wird hier aber nicht weiter beschrieben.
...
**Wichtige Hinweise zu .qmail-Dateien (netqmail) und .mailfilter-Dateien (maildrop) !:** Mit den folgenden zwei Schritten 10.1 und 10.2 wird die Art, in der Mails in Empfang genommen werden, modifiziert. CRM114 wird in dem Empfangsprozess jeder ankommenden E-Mail des Mailaccounts eingebaut und kann so auf Ham und Spam prüfen. Anschließend wird die E-Mail, dem Ergebnis der Prüfung entsprechend, in den Spamerkannt-Ordner oder in den Posteingangs-Ordner gespeichert. Das Einbauen von CRM114 in den Mailempfang erfolgt für jeden Mailaccount einzeln, indem jeweils eine entsprechende .mailfilter-Datei und eine entsprechende .qmail-Datei angelegt werden und anschließend, mit entsprechenden Befehlen gefüllt, verwendet werden. Legt man einen Mailaccount in uberspace über den Befehl `uberspace mail user add [!USERNAME!]` an, so werden *keine* zugehörige .mailfilter-Datei und *keine* zugehörige .qmail-Datei angelegt, da dies für die unmittelbare Zustellung einer ankommenden E-Mail an einen Mailaccount mit gleichem Namen nicht notwendig ist. Will man nun CRM114 in diesen Mailempfangsweg einbauen, so werden die entsprechende .qmail- und .mailfilter-Datei mit den richtigen Inhalten definitiv benötigt. Sofern für den Mailaccount noch keine .mailfilter-Datei und keine .qmail-Datei verwendet werden bzw. angelegt wurde, kann die Anleitung nun einfach weiter verfolgt werden. Die Dateien werden in den folgenden Schritten entsprechend angelegt und gefüllt. Hat man jedoch bereits Zustellregelungen über .qmail- und .mailfilter-Datei für den Mailaccount angelegt, so dürfen die folgenden Schritte nicht 1:1 übernommen werden! Stattdessen müssen die vorhandenen Dateien entsprechend angepasst werden. Wer bereits eigene .qmail- und .mailfilter-Dateien angelegt hat, wird die Erfahrung damit haben und wissen, was zu tun ist. Wer sich unsicher ist, der sollte prüfen, ob im Homeordner .qmail- und .mailfilter-Dateien mit der Benennung .qmail_[!USERNAME!] und .mailfilter-[!USERNAME!] für den jeweilige Mailaccount existieren. Die Prüfung auf Vorhandensein der Datein kann mit der Auflistung aller Dateien des Homeordner mit folgendem Shell-Befehl geschehen: `cd ~ && ls -la`
